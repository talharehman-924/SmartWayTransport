...
export async function getDriverBalances() {
  ...
  (pendingBookings || []).forEach(b => {
    ...
    if (!balances[b.driver_name]) balances[b.driver_name] = { totalPending: 0, totalPaid: 0, netOwed: 0 };
    balances[b.driver_name].totalPending += Number(b.commission_sar) || 0;
  });

  // Sum payments
  (payments || []).forEach(p => {
    if (!p.driver_name) return;
    if (!balances[p.driver_name]) balances[p.driver_name] = { totalPending: 0, totalPaid: 0, netOwed: 0 };
    balances[p.driver_name].totalPaid += Number(p.amount) || 0;
  });

  // Calculate net owed
  Object.keys(balances).forEach(driver => {
    balances[driver].netOwed = balances[driver].totalPending - balances[driver].totalPaid;
  });
  ...
  return Object.keys(balances).map(driverName => ({
    driverName,
    ...balances[driverName]
  })).filter(b => b.totalPending > 0 || b.totalPaid > 0);
}

export async function recordDriverPayment(driverName, amount, method, adminEmail) {
